...
size_t fread_progress_bar(void* ptr, const char* filename, size_t size, const size_t block_size, FILE* stream, unsigned char bar_length, unsigned char head_length, char filled, char empty);
size_t fwrite_progress_bar(void* ptr, const char* filename, size_t size, const size_t block_size, FILE* stream, unsigned char bar_length, unsigned char head_length, char filled, char empty);
...
size_t fread_progress_bar(void* ptr, const char* filename, size_t size, const size_t block_size, FILE* stream, unsigned char bar_length, unsigned char head_length, char filled, char empty)
{
	...
	for(size_t res = 0; transfered < size; rotor = (rotor + 1) % sizeof(rotor_char))
	{
		...
		printf("\r%30s: [%.*s%.03hhu%%%s] %13zu %c", basename((char*)filename), 
